  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20">
      ...
      <Dialog open={!!acceptingRequest} onOpenChange={(open) => !open && setAcceptingRequest(null)}>
        <DialogContent className="rounded-[2.5rem] p-0 sm:max-w-[500px] overflow-hidden border-none shadow-2xl">
          <div className="flex flex-col max-h-[90vh]">
            <div className="flex-grow overflow-y-auto p-8">
              <DialogHeader>
                ...
                <DialogDescription className="text-slate-500 font-medium">You are committing to help with this neighborhood mission.</DialogDescription>
              </DialogHeader>
              <div className="py-6 space-y-6">
                <div className="space-y-2"><h4 className="text-lg font-bold text-slate-900 leading-tight">{acceptingRequest?.title}</h4><p className="text-sm text-slate-500 dark:text-slate-400">{acceptingRequest?.description}</p></div>
                <div className="grid grid-cols-2 gap-4"><div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Category</p><p className="text-sm font-bold capitalize">{acceptingRequest?.category}</p></div><div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Urgency</p><p className={cn("text-sm font-bold capitalize", acceptingRequest?.urgency === 'high' ? 'text-red-600' : 'text-emerald-600')}>{acceptingRequest?.urgency}</p></div></div>
                <div className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl space-y-4 border border-slate-100 dark:border-slate-700">
                  <div className="flex justify-between items-center"><div className="flex items-center gap-3"><Avatar className="h-10 w-10 ring-2 ring-white dark:ring-slate-700 shadow-sm"><AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${acceptingRequest?.createdBy}`} /><AvatarFallback>?</AvatarFallback></Avatar><div><p className="text-xs font-black text-slate-400 uppercase tracking-widest">Requester</p><p className="text-sm font-bold text-slate-900 dark:text-white">{acceptingRequest?.postedByName}</p></div></div><div className="text-right"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Preference</p><Badge variant="outline" className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-[10px] font-bold gap-1.5 h-6">{acceptingRequest?.contactPreference === 'whatsapp' ? <Smartphone className="w-3 h-3" /> : acceptingRequest?.contactPreference === 'call' ? <Phone className="w-3 h-3" /> : <MessageSquare className="w-3 h-3" />}{acceptingRequest?.contactPreference || 'In-App'}</Badge></div></div>
                  {requesterProfile?.phone && acceptingRequest?.contactPreference !== 'in-app' && <div className="flex items-center justify-between gap-3 p-3 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700 animate-in fade-in slide-in-from-top-2"><div className="flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-300"><Phone className="w-4 h-4 text-emerald-500" /> {requesterProfile.phone}</div><span className="text-[10px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-900/50 px-2 py-0.5 rounded-full uppercase">Direct Contact</span></div>}
                </div>
                <div className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-950/20 rounded-2xl border border-amber-100 dark:border-amber-900/50"><AlertTriangle className="w-5 h-5 text-amber-600 shrink-0" /><p className="text-[11px] text-amber-800 dark:text-amber-400 leading-relaxed font-medium">Safety reminder: Coordinate in public areas and verify identity before meeting.</p></div>
              </div>
            </div>
            <DialogFooter className="p-8 pt-4 bg-slate-50 dark:bg-slate-900 border-t dark:border-slate-800 flex flex-row gap-3">
              ...
              <Button className="flex-[2] rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold h-12 shadow-lg shadow-emerald-500/20" onClick={handleAcceptMission} disabled={loading}>{loading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}Confirm & Help</Button>
            </DialogFooter>
          </div>
        </DialogContent>
      </Dialog>
