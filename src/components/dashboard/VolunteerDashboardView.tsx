...
export function VolunteerDashboardView({ profile, user }: { profile: any; user: User }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
      ...
      <Dialog open={showSuccess} onOpenChange={setShowSuccess}><DialogContent className="rounded-[3rem] p-12 text-center"><div className="bg-emerald-100 w-24 h-24 rounded-[2rem] flex items-center justify-center mx-auto mb-8 animate-bounce"><PartyPopper className="w-12 h-12 text-emerald-600" /></div><h2 className="text-4xl font-headline font-bold text-slate-900 mb-4">Great job! 🎉</h2><p className="text-slate-500 mb-8 max-sm mx-auto">You've successfully helped a neighbor. Your impact points have been updated!</p><Button className="w-full h-14 rounded-2xl bg-slate-900 text-white font-bold text-lg" onClick={() => setShowSuccess(false)}>Back to Dashboard</Button></DialogContent></Dialog>
    </div>
  );
}
