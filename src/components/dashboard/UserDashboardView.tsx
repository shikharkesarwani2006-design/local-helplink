          ...
          <Card className="border-none shadow-xl bg-white dark:bg-slate-900 rounded-3xl overflow-hidden group">
            <CardContent className="pt-8 pb-8 flex items-center gap-6 px-8">
              ...
              <div>
                ...
                <h3 className="text-3xl font-black text-slate-900 dark:text-white">{profile?.totalRatingsCount || 0}</h3>
              </div>
            </CardContent>
          </Card>
          <Card className="border-none shadow-xl bg-white dark:bg-slate-900 rounded-3xl overflow-hidden group">
            <CardContent className="pt-8 pb-8 flex items-center gap-6 px-8">
              ...
              <div>
                ...
                <h3 className="text-3xl font-black text-slate-900 dark:text-white">{profile?.totalHelped || 0}</h3>
              </div>
            </CardContent>
          </Card>
          <Card className="border-none shadow-xl bg-white dark:bg-slate-900 rounded-3xl overflow-hidden group">
            <CardContent className="pt-8 pb-8 flex items-center gap-6 px-8">
              ...
              <div>
                ...
                <h3 className="text-3xl font-black text-slate-900 dark:text-white">{profile?.rating?.toFixed(1) || "5.0"}</h3>
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {myRequests?.map((req) => (
                <Card key={req.id} className="rounded-3xl border-none shadow-lg bg-white dark:bg-slate-900 overflow-hidden flex flex-col group">
                  <div className={cn("h-2 w-full", req.status === 'open' ? 'bg-blue-500' : 'bg-amber-500')} />
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      <Badge className={cn("capitalize px-3 py-1 text-[10px] font-black rounded-full", req.status === 'open' ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700")}>
                        {req.status}
                      </Badge>
                  ...
                  <CardContent className="flex-grow">
                    ...
                    {req.status === 'accepted' && req.acceptedBy && <HelperInfo helperId={req.acceptedBy} />}
                  </CardContent>
                  <CardFooter className="pt-4 border-t border-slate-50 flex gap-2">
                    {req.status === 'open' ? (
                      <Button variant="ghost" size="sm" className="flex-1 rounded-xl text-red-500 font-bold" onClick={() => handleCancelRequest(req.id)}>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full lg:w-auto">
              <div className="flex flex-wrap gap-2">
                {categories.map((cat) => (
                  <button key={cat.id} onClick={() => setCategoryFilter(cat.id)} className={cn("flex items-center gap-2 rounded-full px-5 font-bold h-10 border transition-all text-xs", categoryFilter === cat.id ? "bg-slate-900 text-white" : "bg-white text-slate-500")}>
              ...
              <div className="relative w-full md:w-64">
                ...
                <Input placeholder="Search missions..." className="pl-11 h-10 bg-white rounded-full text-xs" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
            </div>
          </div>
          ...
          {isNearbyLoading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredNearby.map((request) => (
                <Card key={request.id} className="group relative overflow-hidden transition-all duration-300 hover:shadow-2xl bg-white rounded-[2rem] border-none flex flex-col h-full">
                  <div className={cn("absolute left-0 top-0 bottom-0 w-1.5", request.urgency === 'high' ? 'bg-red-500' : 'bg-emerald-500')} />
                  <CardHeader className="pb-2 pt-8 pl-8 pr-6">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <Badge className={cn("text-[9px] font-black uppercase", request.urgency === 'high' ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600")}>{request.urgency}</Badge>
                    </div>
                    <CardTitle className="text-lg font-headline font-bold">{request.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="flex-grow pl-8 pr-6">
                    <p className="text-slate-500 text-xs line-clamp-3 mb-4">{request.description}</p>
                    <div className="flex items-center gap-4 text-[10px] font-bold text-slate-400 border-t pt-4">
                      <div className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" />{request.location?.area}</div>
                  ...
                  <CardFooter className="pt-4 pb-8 pl-8 pr-6">
                    <Button className="w-full bg-slate-900 text-white rounded-2xl font-bold" onClick={() => handleOfferHelp(request)}>
